...
class RunMonitor(Monitor):
    ...
    def __init__(self, config):
        super.__init__(RunMonitor,config)
    ...
    def update_trello_board(self):
        ...
        for run in runs:
            ...
            status_list = self.get_status_list(run)
            card = self.trello.get_card_on_board(self.trello_board,run['name'])
            if card is None:
                card = self.trello.add_card(self.trello.add_list(self.trello_board,status_list), run['name'])
                was_moved= True
            else:
                was_moved = self.trello.change_list(card, status_list, skip_list_ids)
            ...
            if status_list == STALLED and was_moved:
                users = [self.trello.client.get_member(mid) for mid in card.member_ids]
                self.send_status_notification(run,lst.name,users)
